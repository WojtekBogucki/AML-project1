...
```{r lr_GD, fig.height=4, fig.width=9, fig.cap="\\label{fig:lr_SGD}Values of loss during training with different learning rates"}
models <- list()
max_iter <- 1000
epsilon <- 1e-4
lr <- c(0.75, 0.5, 0.3, 0.2, 0.1, 0.05, 0.01)
n <- length(lr)
for (i in 1:n){
...
```
On figure \ref{fig:lr_SGD} we can see
...
```{r conv, fig.height=4, fig.width=9,  fig.cap="\\label{fig:lr_SGD}Values of loss during training for different algorithms"}
ggarrange(g1 , g2, nrow=1)
```
